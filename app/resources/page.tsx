...
const AUTHORS = {
  adewale: { name: 'Adewale',     role: 'Network Engineer',  initials: 'AD', color: '#00e5ff' },
  olu:     { name: 'Olu Oluseki', role: 'Cloud & DevOps',    initials: 'OO', color: '#a855f7' },
}
...
const articles = [
  {
    ...
    excerpt:
      "With Nigeria's Data Protection Commission actively enforcing the NDPA and UK/EU-based operations subject to GDPR, the compliance landscape is increasingly complex. We break it down into actionable steps — from data mapping and consent management to breach notification obligations and cross-border data transfer rules.",
  ...
  },
  {
    ...
    excerpt:
      'Both AWS and Microsoft Azure are enterprise-grade cloud platforms — but they serve different strengths. AWS leads in breadth of services and startup ecosystems; Azure excels in hybrid environments and Microsoft-stack integration. Understanding the differences before you migrate can save months of re-architecture and significant cost.',
  ...
  },
  {
    ...
    excerpt:
      'Software-Defined WAN is reshaping how enterprises connect branch offices, cloud services, and remote workers. For organisations with distributed operations — particularly those with pan-African or cross-continental presence — SD-WAN delivers the agility, visibility, and cost savings that MPLS-only networks cannot.',
  ...
  },
  {
    ...
    excerpt:
      'Traditional firewalls filter traffic by port and protocol. Next-Generation Firewalls go further — inspecting application-layer traffic, integrating live threat intelligence feeds, enforcing identity-based policies, and providing SSL/TLS decryption. We compare leading NGFW vendors and explain what to look for in your next procurement.',
  ...
  },
  {
    ...
    excerpt:
      'One of the most consequential technology decisions an organisation makes is whether to build custom software or deploy an off-the-shelf product. Both carry risk — buying locks you into a vendor\'s roadmap; building demands internal capacity. We share the framework our advisory team uses with clients across Africa and the UK.',
  ...
  },
  {
    ...
    readTime: '8 min read',
    date: 'October 2023',
    title: 'Cloud Migration Best Practices: A Technical Guide for Enterprises Moving Workloads to AWS or Azure',
    excerpt:
      'Cloud migration is rarely a "lift and shift" exercise. Done well, it is an opportunity to re-architect for scale, resilience, and security. Done poorly, it creates cloud sprawl, unexpected costs, and new attack surfaces. We share the phased migration methodology our infrastructure team applies on every cloud project.',
  ...
  },
  {
    ...
    excerpt:
      'The endpoint — the laptop, the phone, the remote worker\'s home router — is now the primary battleground in cybersecurity. As remote and hybrid work cements itself as the default, organisations that rely on perimeter-only security are exposing themselves to a new class of threats. EDR, XDR, and zero trust endpoint controls are no longer optional.',
...
const catColors: Record<string, string> = {
  Cybersecurity: 'text-cyan-neon border-cyan-neon/30',
  Infrastructure: 'text-purple-neon border-purple-neon/30',
  Cloud: 'text-cyan-neon border-cyan-neon/30',
  Applications: 'text-magenta-neon border-magenta-neon/30',
  'Digital Transformation': 'text-purple-neon border-purple-neon/30',
  'Managed Services': 'text-cyan-neon border-cyan-neon/30',
}

function CategoryBadge({ cat }: { cat: string }) {
  const cls = catColors[cat] ?? 'text-white/50 border-white/20'
  return (
  ...
  return (
    <>
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="absolute top-1/2 right-0 w-[500px] h-[500px] bg-purple-neon/5 rounded-full blur-[120px] -translate-y-1/2" />
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <h1 className="text-5xl md:text-6xl font-bold text-white leading-[1.02] tracking-tight mb-6">
            Resources &amp; Insights
          </h1>
      ...
      {/* ── FEATURED ARTICLE ── */}
      <section className="pb-8 bg-[#030314]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <Link href={featured.href} className="group block">
            <div className="grid lg:grid-cols-2 border border-white/8 hover:border-white/20 transition-colors duration-300">
              {/* Text */}
              <div className="p-10 lg:p-14 flex flex-col justify-between">
                <div>
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold text-white leading-snug mb-5 group-hover:text-cyan-neon transition-colors">
                    {featured.title}
                  </h2>
                  <p className="text-white/45 leading-relaxed">{featured.excerpt}</p>
                </div>
                <div className="mt-10 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black text-white"
                      style={{ background: `${featured.author.color}30`, border: `1px solid ${featured.author.color}40` }}
                    >
                      {featured.author.initials}
                    </div>
                    <div>
                      <div className="text-white/80 text-xs font-semibold">{featured.author.name}</div>
                      <div className="text-white/30 text-[10px]">{featured.author.role}</div>
                    </div>
                  </div>
              ...
              {/* Visual */}
              <div className="bg-gradient-to-br from-[#050540] to-[#0a0028] min-h-[280px] lg:min-h-0 flex items-center justify-center p-14 relative overflow-hidden">
                <div className="absolute inset-0 bg-grid opacity-40" />
                <div className="relative text-center">
                  <div className="text-6xl lg:text-7xl font-black text-white/5 leading-none tracking-tighter mb-3">
                    ZERO<br />TRUST
                  </div>
      ...
      {/* ── CATEGORY TABS ── */}
      <div className="bg-[#030314] border-b border-white/5 sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
      ...
      {/* ── ALL ARTICLES ── */}
      <section className="py-12 bg-[#030314]">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="divide-y divide-white/5">
            {articles.map((a, i) => (
              <Link
                key={a.href}
                href={a.href}
                className="group flex flex-col md:flex-row gap-6 md:gap-10 py-10 hover:bg-white/[0.015] -mx-6 px-6 transition-colors"
              >
                <span className="text-white/10 text-xs font-mono w-7 shrink-0 mt-1.5">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-4 mb-4">
                    ...
                    <span className="text-white/25 text-xs">{a.readTime}</span>
                  </div>
                  <h3 className="text-lg md:text-xl font-bold text-white/90 leading-snug mb-3 group-hover:text-white transition-colors">
                    {a.title}
                  </h3>
                  <p className="text-white/35 text-sm leading-relaxed line-clamp-2 mb-5">{a.excerpt}</p>
                  <div className="flex items-center gap-2.5">
                    <div
                      className="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black text-white"
                      style={{ background: `${a.author.color}30`, border: `1px solid ${a.author.color}35` }}
                    >
                      {a.author.initials}
                    </div>
                    <span className="text-white/40 text-xs">{a.author.name}</span>
                    <span className="text-white/15 text-xs">·</span>
                    <span className="text-white/25 text-xs">{a.author.role}</span>
                  </div>
                </div>

                <div className="flex md:items-start md:pt-1.5 shrink-0">
                  <svg
                    className="w-5 h-5 text-white/15 group-hover:text-cyan-neon group-hover:translate-x-1 transition-all duration-200"
      ...
      {/* ── TOPICS + NEWSLETTER ── */}
      <section className="py-24 bg-[#04041c] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              <span className="inline-flex items-center gap-2.5 mb-6">
                <span className="w-8 h-px bg-purple-neon" />
                <span className="text-purple-neon text-xs font-semibold tracking-[0.3em] uppercase">Topics</span>
              </span>
              ...
              <div className="flex flex-wrap gap-2.5">
                {topics.map((tag) => (
                  <span
                    key={tag}
                    className="px-3.5 py-1.5 border border-white/8 text-white/35 text-xs cursor-default hover:border-white/25 hover:text-white/60 transition-colors"
                  >
                    {tag}
                  </span>
                ))}
      ...
      {/* ── BOTTOM CTA ── */}
      <section className="py-16 bg-[#030314] border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          ...
          <Link
            href="/contact"
            className="shrink-0 bg-cyan-neon text-navy-950 font-semibold px-8 py-3.5 rounded-sm text-sm tracking-wide hover:bg-white transition-colors duration-200"
          >
